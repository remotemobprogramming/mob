...
- Add automatic and manual co-author attribution. Mob will collect all committers from a WIP branch and add them as co-authors in the final WIP commit. Co-authors can be manually specified using `mob start --with "Bayard Rustin <br@example.org> as bay, ..."`. Using `as` saves Bayard so next time you can `mob start --with "bay"`.
